...
## 12.7 分頁按鈕尺寸與圓角（2025-10-08）

- 所有按鈕高度提高約 1.5 倍：基礎 `.btn` 內距由 `12px 18px` 調整為 `18px 18px`。
- 分頁按鈕 `.tab` 內距由 `10px 18px` 調整為 `15px 18px`。
- 圓角統一調小為「略帶圓角」：`.btn`、`.tab` 的 `border-radius` 由 15-16px 改為 8px。
- Hover 與 Active 狀態一致性：` .tab.active:hover` 必須套用與 `.tab:hover` 相同的動態效果（位移、縮放、陰影、光澤掃光）。
...
- **輸出圖一致性**：匯出 PNG 時，需以當前主題背景與文字色生成，避免深/淺主題下不一致。
